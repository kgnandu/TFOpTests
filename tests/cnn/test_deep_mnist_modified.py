...
'''
Same as deep_mnist but without dropout
'''
...
def test_deep_mnist_modified():
    ...
    for step in range(1, training_steps + 1):
        ...
        sess.run(train_step, feed_dict={x_image: batch_x, y_: batch_y})  # dropout tied to 0.6 for train
        if step % display_step == 0 or step == 1:
            # Calculate batch loss and accuracy
            train_accuracy = sess.run(accuracy, feed_dict={
                x_image: batch_x, y_: batch_y})
    ...
    placeholders = [x_image]
    predictions = [out_node]
    tfp = TensorFlowPersistor(save_dir="deep_mnist_no_dropout", verbose=False)
