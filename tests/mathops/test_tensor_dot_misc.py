...
def test_tensor_dot_misc():
    tensor_dot_misc = TensorDotMisc(feature_size_a=[36, 3, 4, 5], feature_size_b=[5, 5, 3, 4])
    ...
    sess = tf.Session()
    sess.run(init)
    pred = sess.run(out_node, feed_dict={in_node_a: tensor_dot_misc.a_data, in_node_b: tensor_dot_misc.b_data})
    print(pred)
    print(pred.shape)
    '''
